...
public class HomeUseView extends JFrame {
	...
	public HomeUseView(Account account) {
		...
		JLabel logoLabel = new JLabel(
				"<html><span style='color:#2196F3;font-size:28px;font-weight:bold;'>BETA</span> <span style='color:#666;font-size:24px;'>- CINEMAS</span></html>");
		logoLabel.setIconTextGap(10);
		logoLabel.setAlignmentY(Component.CENTER_ALIGNMENT);
		logoComboPanel.add(logoLabel);
		logoComboPanel.add(Box.createHorizontalStrut(16));
		String[] rapList = { "Hà Nội", "TP. Hồ Chí Minh", "An Giang", "Bắc Giang", "Đồng Nai", "Khánh Hòa",
				"Thái Nguyên", "Thanh Hóa", "Vĩnh Phúc", "Lào Cai" };
		...
		rapMap.put("Hà Nội", new String[] { "Beta Thanh Xuân", "Beta Mỹ Đình", "Beta Đan Phượng", "Beta Giải Phóng",
				"Beta Xuân Thủy", "Beta Tây Sơn" });
		rapMap.put("TP. Hồ Chí Minh", new String[] { "Beta Quang Trung", "Beta Ung Văn Khiêm", "Beta Empire Bình Dương",
				"Beta Hồ Tràm", "Beta Tân Uyên" });
		...
		String[] menuItems = { "LỊCH CHIẾU THEO RẠP", "PHIM", "RẠP", "GIÁ VÉ", "TIN MỚI VÀ ƯU ĐÃI", "NHƯỢNG QUYỀN",
				"THÀNH VIÊN" };
		...
		if (movieWithGenreList.size() > 0 && movieWithGenreList != null) {
			for (MovieWithGenres item : movieWithGenreList) {
				...
				if (firstSchedule != null && firstSchedule.getShowDate() != null) {
					if (firstSchedule.getShowDate().isBefore(LocalDate.now()))
						continue;
				...
				public void mouseClicked(java.awt.event.MouseEvent e) {
					...
					if (selectedTabIndex == 0) {
						if (comingSoonView == null)
							comingSoonView = new ComingSoonView(comingMovieInfo);
						setContentPanel(comingSoonView);
					} else if (selectedTabIndex == 1) {
						if (nowShowingView == null)
							nowShowingView = new NowShowingView(nowShowingMovieInfo);
						setContentPanel(nowShowingView);
					} else if (selectedTabIndex == 2) {
						if (specialScreeningView == null)
							specialScreeningView = new SpecialScreeningView();
